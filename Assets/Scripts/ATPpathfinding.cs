...
public class ATPpathfinding : MonoBehaviour
{
    ...
    // origin location/rotation is the physical ATP
    ...
    private void Raycasting() // This function has been commented out, fairly useless...
    {
    ...
    public void Roam()
    {
        if (Time.timeScale != 0)                               // if game not paused
        {
            ...
            if (roamCounter > roamInterval)
            {
                ...
                if (collision.collider != null && collision.collider.name == "Cell Membrane" && collision.distance < 4 ||
                   collision.collider != null && collision.collider.name == "Inner Cell Wall" && collision.distance < 4)
                {
                    if (heading <= 180)
                    {
                        heading = heading + 180;
