...
{
    public GameObject GTPtoGDP;         // GTP to change to a GDP when it attaches to the G-protein
    public GameObject phosphate;
    public static ParticleSystem destructionEffect;	// 'poof' special effect for phosephate
    static float _speed = 5f;
    public Quaternion rotation;
    public bool spin = false;
    private bool docked = false;        // GTP position = Docked G-protein position
    private bool targeting = false;
    public static float randomizer = 0;// is GTP targeting docked G-protein
    ...
    public void FixedUpdate()
    {
        if (Time.timeScale > 0)
        {
            ...
            if (!targeting)//Look for a target
            {
                ...
                GTP_Roam.gtpRoaming(this.gameObject);
                openTarget = GTP_Roam.FindClosest(transform, "DockedG_Protein");
                if (openTarget != null)
                {
            ...
            else if (!docked)
            {
                ...
                {
                    GTP_Roam.gtpRoaming(this.gameObject); // NEW GTP ROAM
                }
                else
                {
                    docked = GTP_Roam.ProceedToVector(this.gameObject, dockingPosition);
                }
                if (docked)
                {
                    Cloak();
    ...
    public void Cloak()        // This is where the GTP changes into the GDP
    {
        transform.GetComponent<CircleCollider2D>().enabled = false;
        transform.GetComponent<Rigidbody2D>().isKinematic = true;
        transform.position = dockingPosition;
        ...
        //Destroy(gameObject);
        //GameObject obj = Instantiate(GTPtoGDP, gameObject.transform.position, Quaternion.identity) as GameObject; // creates a GDP
        obj.transform.GetComponent<CircleCollider2D> ().enabled = false;
		obj.transform.GetComponent<Rigidbody2D>().isKinematic = true;
        obj.transform.position = dockingPosition;
        obj.transform.parent = myTarget;
		myTarget.tag = "OccupiedG_Protein";
		transform.tag = "DockedGTP";
		myTarget = null;

        GameObject phos = Instantiate(phosphate, gameObject.transform.position, Quaternion.identity) as GameObject; // creates a phosphate */
