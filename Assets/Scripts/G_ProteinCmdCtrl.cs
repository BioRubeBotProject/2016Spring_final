...
public class G_ProteinCmdCtrl : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        childGDP = (GameObject)Instantiate(GDP, transform.position + new Vector3(2.8f, 0.0f, 0), Quaternion.identity);
    ...
    private Vector3 GetOffset()
    {
        if (myTarget.GetChild(0).tag == "Left")
        {
            ...
            return myTarget.position + new Vector3(-2.8f, 0.0f, myTarget.position.z);
        }
        else
            return myTarget.position + new Vector3(2.8f, 0.0f, myTarget.position.z);
    }
    ...
    private bool ProceedToTarget()
    {
        ...
        if (deltaDistance < _speed * Time.deltaTime)
        {
            ...
            if (myTarget.GetChild(0).tag == "Left")
            {
                ...
                childGDP.transform.position = childGDP.transform.position - (new Vector3(2.8f, 0.0f, 0.0f) * 2);
            }
        }
