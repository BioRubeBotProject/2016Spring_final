...
public class T_RegCmdCtrl : MonoBehaviour, Roam.CollectObject
{
    ...
    public static bool tutorialWon;
    public float timeoutMaxInterval;
    public float distanceOffset;
    public AudioClip WaitForATP;
    public Sprite newTreg; // change sprite for atp awaiting
    ...
    void FixedUpdate()
    {
        ...
        if (timeoutForInteraction > timeoutMaxInterval)
        {
            ...
            { // If ATP is Tracking this Transcription Regulator
        ...
        else if (tag == "T_Reg_Prep_A")
        {
            ...
            { // If Time delay, is less than 5 seconds keep Roaming
                if (!midpointSet)
                { // If midpoint not set, setup the midpoint between 
        ...
        else if (tag == "T_Reg_Prep_B")
        {
            ...
            if (!midpointAchieved[0] || !midpointAchieved[1])
            {
                ...
                midpointAchieved[1] = Roam.ProceedToVector(this.gameObject, midpoint + new Vector3(0.0f, -1.70f, 0.0f));
            }
            ...
            if (midpointAchieved[0] && midpointAchieved[1])
            {
                ...
                if (active_Kinase_P2.gameObject.transform.parent == null)
                {
                    ...
                    this.gameObject.GetComponent<SpriteRenderer>().sprite = newTreg; // change sprite 
                    this.gameObject.GetComponent<CircleCollider2D>().enabled = true;
                    ...
                    delay = 0;
                    tag = "ATP_tracking";
                    //Play sound to let user know ATP is being tracked
                    AudioSource sound = gameObject.GetComponent<AudioSource>();
                    sound.PlayOneShot(WaitForATP, 1);
                }
            }
            //Increment the timeout variable by delta time
            timeoutForInteraction += Time.deltaTime;
